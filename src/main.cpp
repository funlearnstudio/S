...
std::filesystem::path runtime_root(){
  if(const char*home=std::getenv("SE_HOME"))return home;
  auto installed=executable_dir.parent_path()/"share"/"se";
  if(std::filesystem::exists(installed/"include"))return installed;
#ifdef SE_SOURCE_ROOT
  return SE_SOURCE_ROOT;
#else
...
int file_command(const std::string&cmd,const std::filesystem::path&path){
  ...
  try{auto p=frontend_file(path);if(cmd=="check"){std::cout<<path.string()<<" is valid SE.\n";return 0;}if(cmd=="run"){s::Interpreter vm(std::cin,std::cout);vm.run(p);return 0;}
    auto cpp=s::CppCompiler{}.generate(p);auto cpp_path=path;cpp_path.replace_extension(".se.cpp");auto output=path;output.replace_extension();
#ifdef _WIN32
    output += ".exe";
#endif
    {std::ofstream f(cpp_path);f<<cpp;}
    const char*cxx=std::getenv("CXX");
    auto root=runtime_root();
#ifdef _WIN32
    std::string compiler=cxx?cxx:"cl";
    std::string command;
    if(compiler.find("cl")!=std::string::npos){
      command=compiler+" /nologo /std:c++20 /O2 /EHsc /W4 /WX /I"+shell_quote((root/"include").string())+" "+shell_quote(cpp_path.string())+" "+shell_quote((root/"src/runtime/error.cpp").string())+" "+shell_quote((root/"src/runtime/value.cpp").string())+" "+shell_quote((root/"src/interpreter/interpreter.cpp").string())+" "+shell_quote((root/"src/ffi/ffi.cpp").string())+" /Fe:"+shell_quote(output.string());
    }else{
      command=compiler+" -std=c++20 -O2 -Wall -Wextra -Wpedantic -Werror -I"+shell_quote((root/"include").string())+" "+shell_quote(cpp_path.string())+" "+shell_quote((root/"src/runtime/error.cpp").string())+" "+shell_quote((root/"src/runtime/value.cpp").string())+" "+shell_quote((root/"src/interpreter/interpreter.cpp").string())+" "+shell_quote((root/"src/ffi/ffi.cpp").string())+" -o "+shell_quote(output.string());
    }
#else
...
#endif
    int code=std::system(command.c_str());std::filesystem::remove(cpp_path);if(code!=0)throw std::runtime_error("The C++ compiler could not build the generated program.");std::cout<<"Built "<<output.string()<<"\n";return 0;
...
int bind_command(const std::filesystem::path&definition,const std::filesystem::path&output){
  try{auto files=s::generate_bindings(definition,output);std::cout<<"Generated "<<files.metadata.string()<<'\n'<<"Generated "<<files.header.string()<<'\n'<<"Generated "<<files.source.string()<<'\n';return 0;}
...
int main(int argc,char**argv){
  try{
    std::error_code ec;executable_dir=std::filesystem::absolute(argv[0],ec).parent_path();if(ec)executable_dir=std::filesystem::current_path();
    ...
    if(argc>=3&&std::string(argv[1])=="bind"){
      ...
      std::filesystem::path definition=argv[2];std::filesystem::path output=argc==4?std::filesystem::path(argv[3]):definition.parent_path();if(output.empty())output=".";return bind_command(definition,output);
    }
    if(argc!=3){std::cerr<<"Use: se run file.se | se check file.se | se build file.se | se bind module.sbind [output-directory]\n";return 2;}
    std::filesystem::path file=argv[2];if(file.extension()!=".se"&&file.extension()!=".s"){std::cerr<<"SE source files use the .se extension.\n";return 2;}
    std::string cmd=argv[1];if(cmd!="run"&&cmd!="check"&&cmd!="build"){std::cerr<<"Unknown command '"<<cmd<<"'. Use run, check, build, or bind.\n";return 2;}return file_command(cmd,file);
