...
Value call_native(const std::shared_ptr<void>&lib,const ast::NativeFunction&f,const std::vector<Value>&a,SourcePos p){
  ...
  if(all("Bytes")&&(f.result=="Bytes"||f.result=="Int")){
    std::vector<SBytesView> views;views.reserve(a.size());
    for(auto&x:a){auto b=std::get_if<std::shared_ptr<ByteBufferData>>(&x.data());if(!b)native_fail(p,"Native Bytes argument has the wrong type.");views.push_back({(*b)->bytes.data(),(*b)->bytes.size()});}
    if(f.result=="Int"){
