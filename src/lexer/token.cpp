...
#define S_TOKEN(x) case TokenKind::x: return #x
    ...
    S_TOKEN(Identifier); S_TOKEN(Integer); S_TOKEN(Number); S_TOKEN(String);
    ...
    S_TOKEN(Make); S_TOKEN(Give); S_TOKEN(And); S_TOKEN(Or); S_TOKEN(Not);
    ...
    S_TOKEN(RightBracket); S_TOKEN(Comma); S_TOKEN(Dot); S_TOKEN(Range);
#undef S_TOKEN
  }
  return "unknown";
}
