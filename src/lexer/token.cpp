...
#define S_TOKEN(x) case TokenKind::x: return #x
    ...
    S_TOKEN(Set); S_TOKEN(Map); S_TOKEN(Match); S_TOKEN(Case); S_TOKEN(And); S_TOKEN(Or); S_TOKEN(Not);
    S_TOKEN(Plus); S_TOKEN(Minus); S_TOKEN(Star); S_TOKEN(Slash); S_TOKEN(Percent);
    S_TOKEN(Power); S_TOKEN(Equal); S_TOKEN(PlusEqual); S_TOKEN(MinusEqual); S_TOKEN(StarEqual); S_TOKEN(SlashEqual); S_TOKEN(PercentEqual);
    S_TOKEN(EqualEqual); S_TOKEN(BangEqual);
    ...
    S_TOKEN(RightBracket); S_TOKEN(Comma); S_TOKEN(Colon); S_TOKEN(Dot); S_TOKEN(Range); S_TOKEN(Arrow);
#undef S_TOKEN
  }
  return "unknown";
}
