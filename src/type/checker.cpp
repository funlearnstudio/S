...
TypeInfo Checker::expr(const ast::ExprPtr& e){
  ...
  if(auto x=std::dynamic_pointer_cast<ast::Index>(e)){
    auto v=expr(x->value);
    auto i=expr(x->index);
    if(v.kind==TypeKind::List){
      if(i.kind!=TypeKind::Int) throw Error(x->pos,"A List index needs Int.");
      return v.element?*v.element:TypeInfo{};
    }
    if(v.kind==TypeKind::Map){
      if(i.kind!=TypeKind::Text) throw Error(x->pos,"A Map key needs Text.");
      return v.value?*v.value:TypeInfo{};
    }
  ...
  if(auto x=std::dynamic_pointer_cast<ast::Member>(e)){
    ...
    if(t.kind==TypeKind::Object&&t.object){
      ...
      if(auto m=t.object->methods.find(x->name);m!=t.object->methods.end()){
        if(m->second->params.empty()) return m->second->result;
        TypeInfo r(TypeKind::Function);r.callable=m->second;return r;
      }
    }
    if(t.kind==TypeKind::Module){
      if(auto m=t.members.find(x->name);m!=t.members.end()){
        if(m->second.callable&&m->second.callable->params.empty()) return m->second.callable->result;
        return m->second;
      }
    }
    if(t.kind==TypeKind::File){
      auto sig=member_call(t,x->name,x->pos);
      if(sig->params.empty()) return sig->result;
      TypeInfo r(TypeKind::Function);r.callable=sig;return r;
    }
    throw Error(x->pos,type_text(t)+" has no member named '"+x->name+"'.");
  }
  if(auto x=std::dynamic_pointer_cast<ast::Unary>(e)){
    auto t=expr(x->value);
    if(x->op==TokenKind::Not&&t.kind==TypeKind::Bool) return TypeInfo(TypeKind::Bool);
    if(x->op==TokenKind::Minus&&numeric(t)) return t;
    throw Error(x->pos,"This unary operation does not work with "+type_text(t)+".");
  }
  if(auto x=std::dynamic_pointer_cast<ast::Binary>(e)){
    auto a=expr(x->left);
    auto b=expr(x->right);
    if(x->op==TokenKind::And||x->op==TokenKind::Or){
      if(a.kind!=TypeKind::Unknown&&a.kind!=TypeKind::Bool) throw Error(x->pos,"'and' and 'or' need Bool values.");
      if(b.kind!=TypeKind::Unknown&&b.kind!=TypeKind::Bool) throw Error(x->pos,"'and' and 'or' need Bool values.");
    ...
    if(x->op==TokenKind::Greater||x->op==TokenKind::Less||x->op==TokenKind::GreaterEqual||x->op==TokenKind::LessEqual){
      if(a.kind==TypeKind::Unknown&&numeric(b)) constrain(x->left,b);
      if(b.kind==TypeKind::Unknown&&numeric(a)) constrain(x->right,a);
      if((a.kind!=TypeKind::Unknown&&!numeric(a))||(b.kind!=TypeKind::Unknown&&!numeric(b))) throw Error(x->pos,"Comparisons need numbers.");
      return TypeInfo(TypeKind::Bool);
    }
    if(x->op==TokenKind::Plus&&a.kind==TypeKind::Text&&b.kind==TypeKind::Text) return TypeInfo(TypeKind::Text);
    if(a.kind==TypeKind::Unknown&&numeric(b)){constrain(x->left,b);a=b;}
    if(b.kind==TypeKind::Unknown&&numeric(a)){constrain(x->right,a);b=a;}
  ...
  if(auto x=std::dynamic_pointer_cast<ast::Call>(e)){
    ...
    if(auto m=std::dynamic_pointer_cast<ast::Member>(x->callee)){
      sig=member_call(expr(m->value),m->name,m->pos);
    }else{
      auto c=expr(x->callee);
      if(c.kind!=TypeKind::Function||!c.callable) throw Error(x->pos,"Only a function or method can be called.");
      sig=c.callable;
    }
    ...
    for(std::size_t i=0;i<x->args.size();++i){
      auto got=expr(x->args[i]);
      if(!sig->variadic&&i<sig->params.size()&&!compatible(sig->params[i],got)) throw Error(x->args[i]->pos,"This value needs "+type_text(sig->params[i])+", but you gave it "+type_text(got)+".");
    }
...
void Checker::block(const ast::Block& b){
  scopes_.push_back({});
  for(auto& s:b) stmt(s);
  scopes_.pop_back();
}
...
void Checker::stmt(const ast::StmtPtr& s){
  ...
  if(auto x=std::dynamic_pointer_cast<ast::Assign>(s)){
    auto t=expr(x->value);
    if(auto n=std::dynamic_pointer_cast<ast::Variable>(x->target)){
      put(n->name,t,x->pos);
      if(!x->init.empty()){
        if(t.kind!=TypeKind::Object||!t.object) throw Error(x->pos,"Indented initialization needs a user-defined type.");
        for(auto& i:x->init){
          auto a=std::dynamic_pointer_cast<ast::Assign>(i);
          auto f=a?std::dynamic_pointer_cast<ast::Variable>(a->target):nullptr;
          if(!a||!f) throw Error(i->pos,"Object initialization only contains field assignments.");
          auto field=t.object->fields.find(f->name);
          if(field==t.object->fields.end()) throw Error(f->pos,"'"+t.object->name+"' has no field named '"+f->name+"'.");
          auto got=expr(a->value);
          if(!compatible(field->second,got)) throw Error(a->pos,"'"+f->name+"' needs "+type_text(field->second)+", but you gave it "+type_text(got)+".");
        }
      }
      return;
    }
    if(auto m=std::dynamic_pointer_cast<ast::Member>(x->target)){
      auto o=expr(m->value);
      if(o.kind!=TypeKind::Object||!o.object) throw Error(x->pos,"Only an object field can be assigned with '.'.");
      auto f=o.object->fields.find(m->name);
      if(f==o.object->fields.end()) throw Error(m->pos,"'"+o.object->name+"' has no field named '"+m->name+"'.");
      if(!compatible(f->second,t)) throw Error(x->pos,"'"+m->name+"' needs "+type_text(f->second)+", but you gave it "+type_text(t)+".");
      return;
    }
    if(auto i=std::dynamic_pointer_cast<ast::Index>(x->target)){
      auto c=expr(i->value);
      auto k=expr(i->index);
      if(c.kind==TypeKind::List){
        if(k.kind!=TypeKind::Int) throw Error(i->pos,"A List index needs Int.");
        if(c.element&&!compatible(*c.element,t)) throw Error(x->pos,"This List needs "+type_text(*c.element)+" values.");
        return;
      }
      if(c.kind==TypeKind::Map){
        if(k.kind!=TypeKind::Text) throw Error(i->pos,"A Map key needs Text.");
        if(c.value&&!compatible(*c.value,t)) throw Error(x->pos,"This Map needs "+type_text(*c.value)+" values.");
        return;
      }
      throw Error(x->pos,"Only List or Map items can be assigned by index.");
    }
  ...
  if(auto x=std::dynamic_pointer_cast<ast::For>(s)){
    auto c=expr(x->values);
    scopes_.push_back({});
    if(c.kind==TypeKind::Map){
      if(x->names.size()>2) throw Error(x->pos,"A Map loop uses at most key and value.");
      scopes_.back()[x->names[0]]=TypeInfo(TypeKind::Text);
      if(x->names.size()==2) scopes_.back()[x->names[1]]=c.value?*c.value:TypeInfo{};
    }else if(c.kind==TypeKind::List||c.kind==TypeKind::Set){
      if(x->names.size()!=1) throw Error(x->pos,"List and Set loops use one loop name.");
      scopes_.back()[x->names[0]]=c.element?*c.element:TypeInfo{};
    }else{
      throw Error(x->pos,"for needs a List, Set, Map, or range after 'in'.");
    }
    for(auto& i:x->body) stmt(i);
    scopes_.pop_back();
    return;
  }
  ...
  if(auto x=std::dynamic_pointer_cast<ast::Function>(s)){
    auto f=find(x->name,x->pos);
    auto sig=f.callable;
    if(!sig){sig=std::make_shared<FunctionSig>();sig->params.resize(x->params.size());}
    scopes_.push_back({});
    for(std::size_t i=0;i<x->params.size();++i) scopes_.back()[x->params[i]]=i<sig->params.size()?sig->params[i]:TypeInfo{};
    auto old=current_function_;
    current_function_=sig;
    ++function_depth_;
    for(auto& i:x->body) stmt(i);
    --function_depth_;
    for(std::size_t i=0;i<x->params.size();++i) sig->params[i]=scopes_.back()[x->params[i]];
    if(sig->result.kind==TypeKind::Unknown) sig->result=TypeInfo(TypeKind::None);
    current_function_=old;
    scopes_.pop_back();
    return;
  }
  if(auto x=std::dynamic_pointer_cast<ast::Type>(s)){
    auto type=find(x->name,x->pos);
    auto info=type.object;
    if(!info) throw Error(x->pos,"Internal type registration error.");
    for(auto& f:x->fields) info->fields[f.name]=expr(f.value);
    for(auto& m:x->methods){
      if(!info->methods.contains(m->name)){
        auto sig=std::make_shared<FunctionSig>();
        sig->params.resize(m->params.size());
        info->methods[m->name]=sig;
      }
    }
    for(auto& m:x->methods){
      auto sig=info->methods[m->name];
      scopes_.push_back(info->fields);
      for(std::size_t i=0;i<m->params.size();++i) scopes_.back()[m->params[i]]=sig->params[i];
      auto old=current_function_;
      current_function_=sig;
      ++function_depth_;
      for(auto& i:m->body) stmt(i);
      --function_depth_;
      for(std::size_t i=0;i<m->params.size();++i) sig->params[i]=scopes_.back()[m->params[i]];
      for(auto& f:info->fields){
        if(auto q=scopes_.back().find(f.first);q!=scopes_.back().end()) f.second=q->second;
      }
      if(sig->result.kind==TypeKind::Unknown) sig->result=TypeInfo(TypeKind::None);
      current_function_=old;
      scopes_.pop_back();
...
void Checker::check_module(const ast::Module& m){
  ...
  if(m.native){
    Scope exports;
    for(auto& f:m.native_functions){
      std::vector<TypeInfo> args;
      for(auto& a:f.args) args.push_back(from_native_name(a));
      exports[f.name]=fn(std::move(args),from_native_name(f.result),f.fallible);
    }
    module_exports_[m.name]=std::move(exports);
    return;
  }
  for(auto& s:m.statements){
    if(auto u=std::dynamic_pointer_cast<ast::Use>(s)){
      auto found=module_exports_.find(u->name);
      if(found==module_exports_.end()) throw Error(u->pos,"Module '"+u->name+"' was not loaded.");
      for(auto& [name,t]:found->second){
        if(scopes_.back().contains(name)) throw Error(u->pos,"The name '"+name+"' is provided by more than one module.");
        scopes_.back()[name]=t;
      }
    }
  }
  for(auto& s:m.statements){
    if(auto t=std::dynamic_pointer_cast<ast::Type>(s)){
      if(scopes_.back().contains(t->name)) throw Error(t->pos,"The name '"+t->name+"' already exists.");
      auto info=std::make_shared<UserTypeInfo>();
      info->name=t->name;
      TypeInfo object(TypeKind::Object);object.object=info;scopes_.back()[t->name]=object;
    }else if(auto f=std::dynamic_pointer_cast<ast::Function>(s)){
      if(scopes_.back().contains(f->name)) throw Error(f->pos,"The name '"+f->name+"' already exists.");
      auto sig=std::make_shared<FunctionSig>();sig->params.resize(f->params.size());
      TypeInfo t(TypeKind::Function);t.callable=sig;scopes_.back()[f->name]=t;
    }
  }
  ...
  for(auto& s:m.statements){
    std::string name;
    if(auto t=std::dynamic_pointer_cast<ast::Type>(s)) name=t->name;
    else if(auto f=std::dynamic_pointer_cast<ast::Function>(s)) name=f->name;
    else if(auto a=std::dynamic_pointer_cast<ast::Assign>(s)) if(auto v=std::dynamic_pointer_cast<ast::Variable>(a->target)) name=v->name;
    if(!name.empty()&&name[0]!='_') exports[name]=find(name,s->pos);
  }
  module_exports_[m.name]=std::move(exports);
}

void Checker::check_project(const ast::Program& p){
  module_exports_.clear();
  for(auto& m:p.modules) check_module(m);
}

void Checker::check(const ast::Program& p){
  if(!p.modules.empty()){check_project(p);return;}
  if(scopes_.empty()) scopes_.push_back({});
  install_builtins();
  for(auto& s:p.statements){
    if(auto t=std::dynamic_pointer_cast<ast::Type>(s)){
      if(!scopes_.back().contains(t->name)){
        auto info=std::make_shared<UserTypeInfo>();info->name=t->name;
        TypeInfo obj(TypeKind::Object);obj.object=info;scopes_.back()[t->name]=obj;
      }
    }else if(auto f=std::dynamic_pointer_cast<ast::Function>(s)){
      if(!scopes_.back().contains(f->name)){
        auto sig=std::make_shared<FunctionSig>();sig->params.resize(f->params.size());
        TypeInfo ft(TypeKind::Function);ft.callable=sig;scopes_.back()[f->name]=ft;
      }
    }
  }
  for(auto& s:p.statements) stmt(s);
}
