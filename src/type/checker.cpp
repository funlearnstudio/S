...
TypeInfo fn(std::vector<TypeInfo> p,TypeInfo r,bool fallible=false,bool variadic=false,std::size_t min=0){TypeInfo t(TypeKind::Function);t.callable=std::make_shared<FunctionSig>();t.callable->params=std::move(p);t.callable->result=std::move(r);t.callable->fallible=fallible;t.callable->variadic=variadic;t.callable->min_args=min;return t;}
}

std::string Checker::type_text(const TypeInfo&t){
  switch(t.kind){case TypeKind::Unknown:return "value";case TypeKind::None:return "None";case TypeKind::Int:return "Int";case TypeKind::Num:return "Num";case TypeKind::Text:return "Text";case TypeKind::Bool:return "Bool";case TypeKind::List:return "List";case TypeKind::Map:return "Map";case TypeKind::Set:return "Set";case TypeKind::Function:return "Function";case TypeKind::Object:return t.object?t.object->name:"Object";case TypeKind::Module:return "Module";case TypeKind::Error:return "Error";case TypeKind::Duration:return "Duration";case TypeKind::Time:return "Time";case TypeKind::Path:return "Path";case TypeKind::File:return "File";case TypeKind::NativeHandle:return t.name.empty()?"NativeHandle":t.name;}return "value";
}
bool Checker::compatible(const TypeInfo&a,const TypeInfo&b){
  if(a.kind==TypeKind::Unknown||b.kind==TypeKind::Unknown)return true;if(a.kind==b.kind){
...
TypeInfo Checker::find(const std::string&n,SourcePos p) const{for(auto i=scopes_.rbegin();i!=scopes_.rend();++i)if(auto x=i->find(n);x!=i->end())return x->second;throw Error(p,"I don't know what \""+n+"\" is.","Create it first with "+n+" = value.");}
void Checker::put(const std::string&n,TypeInfo t,SourcePos p){
  for(auto i=scopes_.rbegin();i!=scopes_.rend();++i)if(auto x=i->find(n);x!=i->end()){
    if(!compatible(x->second,t))throw Error(p,"'"+n+"' needs "+type_text(x->second)+", but you gave it "+type_text(t)+".");
    if(x->second.kind==TypeKind::Unknown&&t.kind!=TypeKind::Unknown)x->second=std::move(t);return;
  }
...
TypeInfo Checker::from_native_name(const std::string&n){
  if(n=="Int")return TypeInfo(TypeKind::Int);if(n=="Num")return TypeInfo(TypeKind::Num);if(n=="Bool")return TypeInfo(TypeKind::Bool);if(n=="Text")return TypeInfo(TypeKind::Text);if(n=="None")return TypeInfo(TypeKind::None);
  if(n.rfind("Handle:",0)==0){TypeInfo t(TypeKind::NativeHandle);t.name=n.substr(7);return t;}return TypeInfo(TypeKind::Unknown);
}
...
TypeInfo Checker::expr(const ast::ExprPtr&e){
  ...
  if(auto x=std::dynamic_pointer_cast<ast::Binary>(e)){
    auto a=expr(x->left),b=expr(x->right);
  ...
  if(auto x=std::dynamic_pointer_cast<ast::Call>(e)){
    ...
    if(auto m=std::dynamic_pointer_cast<ast::Member>(x->callee))sig=member_call(expr(m->value),m->name,m->pos);else{auto c=expr(x->callee);if(c.kind!=TypeKind::Function||!c.callable)throw Error(x->pos,"Only a function or method can be called.");sig=c.callable;}
    ...
    if(sig->fallible&&propagate_depth_>0&&current_function_)current_function_->fallible=true;return sig->result;
  }
  return TypeInfo{};
}
...
void Checker::stmt(const ast::StmtPtr&s){
  ...
  if(auto x=std::dynamic_pointer_cast<ast::Function>(s)){
    auto f=find(x->name,x->pos);auto sig=f.callable;if(!sig){sig=std::make_shared<FunctionSig>();sig->params.resize(x->params.size());}
    scopes_.push_back({});for(std::size_t i=0;i<x->params.size();++i)scopes_.back()[x->params[i]]=i<sig->params.size()?sig->params[i]:TypeInfo{};
    auto old=current_function_;current_function_=sig;++function_depth_;for(auto&i:x->body)stmt(i);--function_depth_;
    for(std::size_t i=0;i<x->params.size();++i)sig->params[i]=scopes_.back()[x->params[i]];if(sig->result.kind==TypeKind::Unknown)sig->result=TypeInfo(TypeKind::None);current_function_=old;scopes_.pop_back();return;
  }
  if(auto x=std::dynamic_pointer_cast<ast::Type>(s)){
    auto type=find(x->name,x->pos);auto info=type.object;if(!info)throw Error(x->pos,"Internal type registration error.");
    for(auto&f:x->fields)info->fields[f.name]=expr(f.value);
    for(auto&m:x->methods)if(!info->methods.contains(m->name)){auto sig=std::make_shared<FunctionSig>();sig->params.resize(m->params.size());info->methods[m->name]=sig;}
    for(auto&m:x->methods){auto sig=info->methods[m->name];scopes_.push_back(info->fields);for(std::size_t i=0;i<m->params.size();++i)scopes_.back()[m->params[i]]=sig->params[i];auto old=current_function_;current_function_=sig;++function_depth_;for(auto&i:m->body)stmt(i);--function_depth_;for(std::size_t i=0;i<m->params.size();++i)sig->params[i]=scopes_.back()[m->params[i]];for(auto&f:info->fields)if(auto q=scopes_.back().find(f.first);q!=scopes_.back().end())f.second=q->second;if(sig->result.kind==TypeKind::Unknown)sig->result=TypeInfo(TypeKind::None);current_function_=old;scopes_.pop_back();}return;
  }
...
void Checker::check_module(const ast::Module&m){
  ...
  if(m.native){Scope exports;for(auto&f:m.native_functions){std::vector<TypeInfo> args;for(auto&a:f.args)args.push_back(from_native_name(a));auto t=fn(std::move(args),from_native_name(f.result),f.fallible);exports[f.name]=t;}module_exports_[m.name]=std::move(exports);return;}
  for(auto&s:m.statements)if(auto u=std::dynamic_pointer_cast<ast::Use>(s)){
    auto found=module_exports_.find(u->name);if(found==module_exports_.end())throw Error(u->pos,"Module '"+u->name+"' was not loaded.");
    for(auto&[name,t]:found->second){if(scopes_.back().contains(name))throw Error(u->pos,"The name '"+name+"' is provided by more than one module.");scopes_.back()[name]=t;}
  }
