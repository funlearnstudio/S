...
std::string Checker::type_text(const TypeInfo& t){
  switch(t.kind){
    ...
    case TypeKind::Generic:return t.name.empty()?"generic":t.name;
...
bool Checker::compatible(const TypeInfo& a,const TypeInfo& b){
  if(a.kind==TypeKind::Unknown||b.kind==TypeKind::Unknown||a.kind==TypeKind::Generic||b.kind==TypeKind::Generic)return true;
  if(a.kind==b.kind){
    if(a.kind==TypeKind::Object){
      if(a.object&&b.object)return a.object->name==b.object->name;
...
void Checker::constrain(const ast::ExprPtr& e,const TypeInfo& t){if(auto v=std::dynamic_pointer_cast<ast::Variable>(e))put(v->name,t,v->pos);}

void Checker::install_builtins(){
  auto& g=scopes_.front();if(g.contains("read"))return;
...
TypeInfo Checker::builtin_module(const std::string& name) const{
  ...
  if(name=="path"){
    ...
    m.members["name"]=fn({TypeInfo(TypeKind::Unknown)},TypeInfo(TypeKind::Text));m.members["ext"]=fn({TypeInfo(TypeKind::Unknown)},TypeInfo(TypeKind::Text));m.members["parent"]=fn({TypeInfo(TypeKind::Unknown)},TypeInfo(TypeKind::Path));m.members["exists"]=fn({TypeInfo(TypeKind::Unknown)},TypeInfo(TypeKind::Bool));m.members["is_file"]=fn({TypeInfo(TypeKind::Unknown)},TypeInfo(TypeKind::Bool));m.members["is_dir"]=fn({TypeInfo(TypeKind::Unknown)},TypeInfo(TypeKind::Bool));
  }else if(name=="time")m.members["now"]=fn({},TypeInfo(TypeKind::Time));
  else if(name=="file"){
    m.members["read"]=fn({TypeInfo(TypeKind::Unknown)},TypeInfo(TypeKind::Text),true);m.members["write"]=fn({TypeInfo(TypeKind::Unknown),TypeInfo(TypeKind::Text)},TypeInfo(TypeKind::None),true);m.members["append"]=fn({TypeInfo(TypeKind::Unknown),TypeInfo(TypeKind::Text)},TypeInfo(TypeKind::None),true);m.members["open"]=fn({TypeInfo(TypeKind::Unknown)},TypeInfo(TypeKind::File),true);
  }else if(name=="math"){
    m.members["pi"]=TypeInfo(TypeKind::Num);m.members["sqrt"]=fn({TypeInfo(TypeKind::Num)},TypeInfo(TypeKind::Num));m.members["abs"]=fn({TypeInfo(TypeKind::Num)},TypeInfo(TypeKind::Num));m.members["floor"]=fn({TypeInfo(TypeKind::Num)},TypeInfo(TypeKind::Num));m.members["ceil"]=fn({TypeInfo(TypeKind::Num)},TypeInfo(TypeKind::Num));m.members["round"]=fn({TypeInfo(TypeKind::Num)},TypeInfo(TypeKind::Num));m.members["pow"]=fn({TypeInfo(TypeKind::Num),TypeInfo(TypeKind::Num)},TypeInfo(TypeKind::Num));m.members["min"]=fn({TypeInfo(TypeKind::Num),TypeInfo(TypeKind::Num)},TypeInfo(TypeKind::Num));m.members["max"]=fn({TypeInfo(TypeKind::Num),TypeInfo(TypeKind::Num)},TypeInfo(TypeKind::Num));
  }else if(name=="random"){
    m.members["int"]=fn({TypeInfo(TypeKind::Int),TypeInfo(TypeKind::Int)},TypeInfo(TypeKind::Int));m.members["num"]=fn({},TypeInfo(TypeKind::Num));
  }else if(name=="os"){
    m.members["platform"]=TypeInfo(TypeKind::Text);m.members["cwd"]=fn({},TypeInfo(TypeKind::Path));m.members["getenv"]=fn({TypeInfo(TypeKind::Text)},TypeInfo(TypeKind::Text));m.members["has_env"]=fn({TypeInfo(TypeKind::Text)},TypeInfo(TypeKind::Bool));
  }
  return m;
}

TypeInfo Checker::from_native_name(const std::string& n){
  if(n=="Int")return TypeInfo(TypeKind::Int);if(n=="Num")return TypeInfo(TypeKind::Num);if(n=="Bool")return TypeInfo(TypeKind::Bool);if(n=="Text")return TypeInfo(TypeKind::Text);if(n=="Bytes")return TypeInfo(TypeKind::Bytes);if(n=="None")return TypeInfo(TypeKind::None);if(n.rfind("Handle:",0)==0){TypeInfo t(TypeKind::NativeHandle);t.name=n.substr(7);return t;}return TypeInfo(TypeKind::Unknown);
}

std::shared_ptr<FunctionSig> Checker::member_call(const TypeInfo& t,const std::string& name,SourcePos p) const{
  if(t.kind==TypeKind::Object&&t.object){if(auto i=t.object->methods.find(name);i!=t.object->methods.end())return i->second;}
  if(t.kind==TypeKind::Module){if(auto i=t.members.find(name);i!=t.members.end()&&i->second.callable)return i->second.callable;}
  if(t.kind==TypeKind::List||t.kind==TypeKind::Set){TypeInfo elem=t.element?*t.element:TypeInfo{};if(name=="add")return fn({elem},TypeInfo(TypeKind::None)).callable;if(name=="remove")return fn({elem},TypeInfo(TypeKind::Bool)).callable;}
  if(t.kind==TypeKind::File){if(name=="read")return fn({},TypeInfo(TypeKind::Text),true).callable;if(name=="close")return fn({},TypeInfo(TypeKind::None)).callable;if(name=="write")return fn({TypeInfo(TypeKind::Text)},TypeInfo(TypeKind::None),true).callable;}
...
TypeInfo Checker::expr(const ast::ExprPtr& e){
  if(auto x=std::dynamic_pointer_cast<ast::Literal>(e)){switch(x->value.index()){case 0:return TypeInfo(TypeKind::Int);case 1:return TypeInfo(TypeKind::Num);case 2:return TypeInfo(TypeKind::Text);default:return TypeInfo(TypeKind::Bool);}}
  ...
  if(auto x=std::dynamic_pointer_cast<ast::List>(e)){TypeInfo elem;for(auto& i:x->items){auto t=expr(i);if(elem.kind==TypeKind::Unknown)elem=t;else if(!compatible(elem,t))throw Error(i->pos,"A List keeps one value type. It already has "+type_text(elem)+", but this is "+type_text(t)+".");}return list_of(elem);}
  if(auto x=std::dynamic_pointer_cast<ast::Set>(e)){TypeInfo elem;for(auto& i:x->items){auto t=expr(i);if(elem.kind==TypeKind::Unknown)elem=t;else if(!compatible(elem,t))throw Error(i->pos,"A Set keeps one value type. It already has "+type_text(elem)+", but this is "+type_text(t)+".");}return set_of(elem);}
  if(auto x=std::dynamic_pointer_cast<ast::Map>(e)){TypeInfo value;for(auto& i:x->items){auto k=expr(i.first);if(k.kind!=TypeKind::Text)throw Error(i.first->pos,"Map keys are Text in SE.");auto v=expr(i.second);if(value.kind==TypeKind::Unknown)value=v;else if(!compatible(value,v))throw Error(i.second->pos,"A Map keeps one value type. It already has "+type_text(value)+", but this is "+type_text(v)+".");}return map_of(TypeInfo(TypeKind::Text),value);}
  if(auto x=std::dynamic_pointer_cast<ast::Ask>(e)){if(expr(x->question).kind!=TypeKind::Text)throw Error(x->pos,"ask needs Text.");return TypeInfo(TypeKind::Text);}
  if(auto x=std::dynamic_pointer_cast<ast::TryExpr>(e)){++propagate_depth_;auto t=expr(x->value);--propagate_depth_;if(current_function_)current_function_->fallible=true;return t;}
  if(auto x=std::dynamic_pointer_cast<ast::Range>(e)){if(expr(x->start).kind!=TypeKind::Int||expr(x->end).kind!=TypeKind::Int)throw Error(x->pos,"A range needs two Int values.");return list_of(TypeInfo(TypeKind::Int));}
  if(auto x=std::dynamic_pointer_cast<ast::Index>(e)){auto v=expr(x->value);auto i=expr(x->index);if(v.kind==TypeKind::List){if(i.kind!=TypeKind::Int)throw Error(x->pos,"A List index needs Int.");return v.element?*v.element:TypeInfo{};}if(v.kind==TypeKind::Map){if(i.kind!=TypeKind::Text)throw Error(x->pos,"A Map key needs Text.");return v.value?*v.value:TypeInfo{};}throw Error(x->pos,"Only List and Map can use [index].");}
  if(auto x=std::dynamic_pointer_cast<ast::Member>(e)){
    auto t=expr(x->value);if(x->name=="help")return TypeInfo(TypeKind::Text);if(x->name=="len"&&(t.kind==TypeKind::Text||t.kind==TypeKind::Bytes||t.kind==TypeKind::List||t.kind==TypeKind::Map||t.kind==TypeKind::Set))return TypeInfo(TypeKind::Int);if((x->name=="upper"||x->name=="lower")&&t.kind==TypeKind::Text)return TypeInfo(TypeKind::Text);
    if(t.kind==TypeKind::Object&&t.object){if(auto f=t.object->fields.find(x->name);f!=t.object->fields.end())return f->second;if(auto m=t.object->methods.find(x->name);m!=t.object->methods.end()){if(m->second->params.empty())return m->second->result;TypeInfo r(TypeKind::Function);r.callable=m->second;return r;}}
...
void Checker::stmt(const ast::StmtPtr& s){
  ...
  if(auto x=std::dynamic_pointer_cast<ast::Assign>(s)){
    auto t=expr(x->value);if(auto n=std::dynamic_pointer_cast<ast::Variable>(x->target)){put(n->name,t,x->pos);if(!x->init.empty()){if(t.kind!=TypeKind::Object||!t.object)throw Error(x->pos,"Indented initialization needs a user-defined type.");for(auto& i:x->init){auto a=std::dynamic_pointer_cast<ast::Assign>(i);auto f=a?std::dynamic_pointer_cast<ast::Variable>(a->target):nullptr;if(!a||!f)throw Error(i->pos,"Object initialization only contains field assignments.");auto field=t.object->fields.find(f->name);if(field==t.object->fields.end())throw Error(f->pos,"'"+t.object->name+"' has no field named '"+f->name+"'.");auto got=expr(a->value);if(!compatible(field->second,got))throw Error(a->pos,"'"+f->name+"' needs "+type_text(field->second)+", but you gave it "+type_text(got)+".");}}return;}if(auto m=std::dynamic_pointer_cast<ast::Member>(x->target)){auto o=expr(m->value);if(o.kind!=TypeKind::Object||!o.object)throw Error(x->pos,"Only an object field can be assigned with '.'.");auto f=o.object->fields.find(m->name);if(f==o.object->fields.end())throw Error(m->pos,"'"+o.object->name+"' has no field named '"+m->name+"'.");if(!compatible(f->second,t))throw Error(x->pos,"'"+m->name+"' needs "+type_text(f->second)+", but you gave it "+type_text(t)+".");return;}if(auto i=std::dynamic_pointer_cast<ast::Index>(x->target)){auto c=expr(i->value),k=expr(i->index);if(c.kind==TypeKind::List){if(k.kind!=TypeKind::Int)throw Error(i->pos,"A List index needs Int.");if(c.element&&!compatible(*c.element,t))throw Error(x->pos,"This List needs "+type_text(*c.element)+" values.");return;}if(c.kind==TypeKind::Map){if(k.kind!=TypeKind::Text)throw Error(i->pos,"A Map key needs Text.");if(c.value&&!compatible(*c.value,t))throw Error(x->pos,"This Map needs "+type_text(*c.value)+" values.");return;}throw Error(x->pos,"Only List or Map items can be assigned by index.");}throw Error(x->pos,"You can only assign to a name, field, or collection item.");
  }
  ...
  if(auto x=std::dynamic_pointer_cast<ast::Match>(s)){auto subject=expr(x->value);for(auto& c:x->cases){auto pattern=expr(c.pattern);if(!compatible(subject,pattern))throw Error(c.pos,"This case has type "+type_text(pattern)+", but match value is "+type_text(subject)+".");block(c.body);}block(x->else_block);return;}
  ...
  if(auto x=std::dynamic_pointer_cast<ast::Type>(s)){
    auto type=find(x->name,x->pos);auto info=type.object;if(!info)throw Error(x->pos,"Internal type registration error.");for(auto& f:x->fields)info->fields[f.name]=expr(f.value);for(auto& m:x->methods)if(!info->methods.contains(m->name)){auto sig=std::make_shared<FunctionSig>();configure_signature(*m,sig);info->methods[m->name]=sig;}for(auto& m:x->methods){auto sig=info->methods[m->name];scopes_.push_back(info->fields);for(std::size_t i=0;i<m->params.size();++i)scopes_.back()[m->params[i]]=sig->params[i];auto old=current_function_;current_function_=sig;++function_depth_;for(auto& i:m->body)stmt(i);--function_depth_;for(std::size_t i=0;i<m->params.size();++i)if(sig->params[i].kind==TypeKind::Unknown)sig->params[i]=scopes_.back()[m->params[i]];for(auto& f:info->fields)if(auto q=scopes_.back().find(f.first);q!=scopes_.back().end())f.second=q->second;if(sig->result.kind==TypeKind::Unknown)sig->result=TypeInfo(TypeKind::None);current_function_=old;scopes_.pop_back();}return;
  }
...
void Checker::check_module(const ast::Module& m){
  scopes_.clear();scopes_.push_back({});install_builtins();if(m.builtin){module_exports_[m.name]={{m.name,builtin_module(m.name)}};return;}if(m.native){Scope exports;for(auto& f:m.native_functions){std::vector<TypeInfo> args;for(auto& a:f.args)args.push_back(from_native_name(a));exports[f.name]=fn(std::move(args),from_native_name(f.result),f.fallible);}module_exports_[m.name]=std::move(exports);return;}
